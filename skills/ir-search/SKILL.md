...
2. 새 보고서 폴더에 jsonl 저장 후 비교 — 프로필 스냅샷이 있으면 반드시 함께 넘긴다:
   ...
   ```
3. **검토·상세검증은 `new_items.jsonl`(신규 + 변경 + 새 소스분)만** 한다. UNCHANGED 항목은 직전 보고서의 A/B/C 판정을 그대로 승계하고 재검증하지 않는다
4. **프로필의 판정 축(창업 단계·지역·연령 등)이 바뀌었으면 UNCHANGED 승계 금지 — 전수 재검토한다.** diff 스크립트가 fingerprint로 이를 검증하며, 프로필 인자를 한쪽만 줬거나 fingerprint가 다르면 "CARRY-OVER INVALIDATED"를 출력하고 전건을 `--out`에 기록한다
5. CHANGED(제목·기간·상태 변경)는 changed_fields를 보고 판단: 마감일만 연장이면 판정 유지 + 마감일 갱신, 단 이전 A그룹 건이면 상세를 재확인 (연장 공고는 자격요건 변경이 동반되기도 한다)
6. 보고서는 증분 구조로: **신규 공고 (A/B/C 분류) / 변경 (changed_fields 명시) / 종료된 공고 중 직전 A그룹이던 것 (기회 소멸 알림) / 승계 요약 (직전 A그룹 현황 + 남은 마감)**. 직전 보고서 경로를 상단에 링크한다
7. **조사 완료 시 사용한 프로필 사본을 보고서 폴더에 `ir-profile-snapshot.md`로 저장한다** — 다음 diff의 `--old-profile` 입력이 된다
