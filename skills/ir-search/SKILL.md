...
> **스크립트 위치**: 크롤러·유틸리티는 플러그인 디렉토리 아래 `${CLAUDE_PLUGIN_ROOT}/skills/ir-search/scripts/` 에 있다(단독 스킬 설치 시에는 스킬 디렉토리 자체). 아래 코드 블록은 이미 절대경로로 표기했으니 `${CLAUDE_PLUGIN_ROOT}`(Claude Code) 또는 각 에이전트의 플러그인 루트 변수로 치환해 실행한다. 변수를 모르면 스킬 디렉토리를 먼저 식별할 것.
